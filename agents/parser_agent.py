...
class StructuredOutput(BaseModel):
    problem_text: str = Field(description=
        'The cleaned and fully structured text of the math problem')
    topic: str = Field(description=
        'The main topic of the math problem, e.g. algebra, probability, calculus, linear_algebra'
        )
    variables: List[str] = Field(description=
        'List of math variables present in the problem, like x, y, a, b')
    constraints: List[str] = Field(description=
        "List of constraints or equations given in the problem, e.g., 'x > 0', 'a + b = 5'"
        )
    needs_clarification: bool = Field(description=
        'True if the input is too ambiguous or missing crucial information to be a solvable problem, otherwise False'
        )
