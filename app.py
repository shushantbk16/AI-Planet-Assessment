...
load_dotenv()
st.set_page_config(page_title='Math Mentor — AI Planet', page_icon='🧮',
    layout='wide')
st.markdown(
    """
<style>
...
for key, default in [('parsed_problem', None), ('final_state', None), (
    'memory_record_id', None), ('extracted_text', ''), ('ocr_confidence',
    None), ('transcript', ''), ('asr_unclear', False)]:
...
def process_problem(raw_text: str, input_mode: str='text', original_input:
    str=''):
    st.session_state.final_state = None
    with st.spinner('🔍 Parser Agent — structuring the problem…'):
        try:
            parsed_result = parser_agent.invoke({'input_text': raw_text})
            result_dict = parsed_result.dict() if hasattr(parsed_result, 'dict'
                ) else parsed_result
            ...
            if result_dict.get('needs_clarification'):
                st.warning(
                    '⚠️ **HITL triggered** — the parser detected ambiguity. Please edit and re-submit your input.'
                    )
                return
    ...
    if similar:
        with st.expander(
            f'💡 Memory — {len(similar)} similar solved problem(s) found'):
            for s in similar:
                st.markdown(
                    f"**Problem:** {s['parsed_problem'].get('problem_text', '')}"
                    )
    ...
    with st.spinner('🤖 Running multi-agent workflow…'):
        try:
            ...
            for output in workflow.stream({'parsed_problem': result_dict,
                'retry_count': 0, 'agent_trace': [], 'similar_problems':
                similar}):
                for node_name, value in output.items():
                    st.toast(f'Agent finished: **{node_name}**', icon='✅')
                    final_value = value
            st.session_state.final_state = final_value
            record_id = save_attempt(parsed_problem=result_dict,
                rag_context=final_value.get('rag_context', ''),
                solution_draft=final_value.get('solution_draft', ''),
                verification_passed=final_value.get('verification_passed', 
                False), final_explanation=final_value.get(
                'final_explanation', ''), original_input=original_input,
...
def display_results():
    ...
    if state.get('final_explanation'):
        ...
        st.warning(
            'The verifier could not confirm this answer after 2 attempts. Please review the Solver Draft above and correct manually.'
            )
    st.divider()
    ...
    with col_b:
        comment = st.text_input('Describe the error or hint for retry:',
            key='fb_comment')
        col_b1, col_b2 = st.columns(2)
        with col_b1:
            if st.button('❌ Incorrect (Just Log)', key='btn_incorrect',
                use_container_width=True):
                fb = f'incorrect:{comment}' if comment else 'incorrect'
                if st.session_state.memory_record_id:
                    update_feedback(st.session_state.memory_record_id, fb)
                st.warning(
                    'Logged as incorrect — will improve future attempts!')
        with col_b2:
            if st.button('🔁 Re-Calculate (HITL)', key='btn_recalc', type=
                'primary', use_container_width=True):
                if not comment:
                    st.error(
                        'Please provide a hint or correction in the text box for the retry.'
                        )
                else:
                    st.info(
                        'Triggering explicit re-check with your feedback...')
                    ...
                    if last_state:
                        ...
                        with st.spinner(
                            '🤖 Re-running solver with your feedback...'):
                            final_value = {}
                            for output in workflow.stream(last_state):
                                for node_name, value in output.items():
                                    ...
                                    final_value = value
                            st.session_state.final_state = final_value
                            record_id = save_attempt(parsed_problem=
                                final_value.get('parsed_problem', {}),
                                rag_context=final_value.get('rag_context',
                                ''), solution_draft=final_value.get(
                                'solution_draft', ''), verification_passed=
                                final_value.get('verification_passed', 
                                False), final_explanation=final_value.get(
                                'final_explanation', ''), original_input=
                                'recalc_from_feedback', input_mode='text')
                            st.session_state.memory_record_id = record_id
                            st.rerun()


st.divider()
input_mode = st.radio('**Select Input Mode**', ['✏️ Text', '🖼️ Image',
    '🎙️ Audio'], horizontal=True)
if input_mode == '✏️ Text':
    ...
    if st.button('🚀 Solve', key='solve_text'):
        if user_text.strip():
            process_problem(user_text.strip(), input_mode='text',
                original_input=user_text.strip())
        else:
    ...
    if uploaded_image:
        ...
        if st.session_state.extracted_text:
            text_to_show = apply_correction_rules(st.session_state.
            ...
            if st.session_state.ocr_confidence is not None:
                ...
                if is_low_confidence(st.session_state.ocr_confidence):
                    st.warning(
                        '⚠️ **HITL triggered** — OCR confidence is low. Please review and correct the text below.'
                        )
            edited = st.text_area('Extracted Text (edit if needed):', value
                =text_to_show, height=150)
            if st.button('🚀 Solve Image Problem', key='solve_image'):
                save_correction_rule(st.session_state.extracted_text,
                    edited, 'image')
                process_problem(edited, input_mode='image', original_input=
                    st.session_state.extracted_text)
elif input_mode == '🎙️ Audio':
    st.info(
        'You can either upload an audio file or record directly using your microphone.'
        )
    col1, col2 = st.columns(2)
    with col1:
    ...
    if audio_source:
        ...
        if st.session_state.transcript:
            trans_to_show = apply_correction_rules(st.session_state.
                transcript, 'audio')
            if st.session_state.asr_unclear:
                st.warning(
                    '⚠️ **HITL triggered** — transcription confidence is low. Please review and correct below.'
                    )
            edited_t = st.text_area('Transcript (edit if needed):', value=
                trans_to_show, height=150)
            if st.button('🚀 Solve Audio Problem', key='solve_audio'):
                save_correction_rule(st.session_state.transcript, edited_t,
                    'audio')
                process_problem(edited_t, input_mode='audio',
                    original_input=st.session_state.transcript)
display_results()
